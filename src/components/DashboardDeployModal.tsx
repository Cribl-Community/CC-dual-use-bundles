...
import type { Dashboard, DetectionForDashboard } from '../api/dashboards';
...
interface Props {
  sourceName: string;
  sourceId: string;
  secDetections: DetectionForDashboard[];
  obsDetections: DetectionForDashboard[];
...
export default function DashboardDeployModal({ sourceName, sourceId, secDetections, obsDetections, enabledSecIds, enabledObsIds, onClose }: Props) {
  ...
  async function deploy() {
    const apiUrlVal = (window as unknown as { CRIBL_API_URL?: string }).CRIBL_API_URL ?? '';

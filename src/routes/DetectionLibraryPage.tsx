...
export default function DetectionLibraryPage() {
  const sources = useMemo(() => dataSources.flatMap(c => c.sources).sort((a, b) => a.name.localeCompare(b.name)), []);
  const [selectedSource, setSelectedSource] = useState(sources.find(s => s.status === 'available')?.id || 'palo-alto-traffic');
  ...
  const securityDetections = useMemo(() => secDetData[selectedSource] ?? [], [selectedSource]);
  const obsDetections = useMemo(() => obsDetData[selectedSource] ?? [], [selectedSource]);
  const allFields = useMemo(() => fieldMatrix[selectedSource] ?? [], [selectedSource]);
  const enrichmentData = enrichmentDataAll[selectedSource] ?? null;
  ...
  function selectAllSec() { setEnabledSecDetections(new Set(securityDetections.map(d => d.id))); }
  ...
  function selectAllStreamEnrichments() { if (enrichmentData) setEnabledStreamEnrichments(new Set(enrichmentData.streamTime.map(e => e.name))); }
  function selectAllSearchEnrichments() { if (enrichmentData?.searchTime) setEnabledSearchEnrichments(new Set(enrichmentData.searchTime.map(e => e.name))); }
  ...
  const fieldAnalysis = useMemo(() => {
    ...
    securityDetections.filter(d => enabledSecDetections.has(d.id)).forEach(d => d.requiredFields.forEach(f => secRequired.add(f)));
    ...
    const notNeeded = allFields.map(f => f.field).filter(f => !allRequired.has(f));
  ...
  const filteredDetections = detections.filter(d => {
    if (search) {
      ...
      if (!d.name.toLowerCase().includes(s) && !d.objective.toLowerCase().includes(s) && !d.requiredFields.some(f => f.includes(s))) return false;
    }
  ...
  async function exportSearchPack() {
    ...
    [...securityDetections.filter(d => enabledSecDetections.has(d.id)),
  ...
  async function exportStreamPack() {
    const selectedEnrichments = enrichmentData?.streamTime?.filter(e => enabledStreamEnrichments.has(e.name)) ?? [];
  ...
  async function deployToCriblSearch(dataset: string) {
    ...
    const selectedSearchEnrich = enrichmentData?.searchTime?.filter(e => enabledSearchEnrichments.has(e.name)) ?? [];
    ...
    for (const det of selectedDets) {
      ...
      for (const q of det.criblSearchQueries) {
        ...
        try {
          ...
          errorCount++;
          log(`! Error: ${body.name} — ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    }
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'var(--cds-font-size-sm)' }}>
                        ...
                        <tbody>
                          {fieldAnalysis.notNeeded.map((fieldName: string) => {
                            const f = allFields.find(af => af.field === fieldName);
                            if (!f) return null;
                            return (
        ...
        {activeTab === 'enrichments' && enrichmentData && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 16 }}>
              {(enrichmentSubTab === 'stream' ? enrichmentData.streamTime : enrichmentData.searchTime ?? []).map(e => {

...
export default function FieldMatrixPage() {
  const sources = useMemo(() =>
    dataSources.flatMap(c => c.sources).filter(s => s.status === 'available'),
    []
  );
  ...
  const securityCount = fields.filter(f => f.securitySiem === 'Yes').length;
  ...
  const maskableCount = fields.filter(f => f.canMask === 'Yes' || f.canMask === 'Sometimes').length;

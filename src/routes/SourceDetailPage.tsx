...
const personaLenses: Record<string, { label: string; color: string; filter: (j: { persona: string }) => boolean; description: string }> = {
  all: { label: 'All Teams', color: 'var(--cds-brand-teal)', filter: () => true, description: 'All use cases across every team' },
  soc: { label: 'SOC', color: 'var(--cds-color-danger)', filter: j => /SOC|Security|Incident|Threat/i.test(j.persona), description: 'Security Operations — detection, investigation, response' },
  platform: { label: 'Platform Eng', color: 'var(--cds-color-accent)', filter: j => /Platform|DevOps|SRE|Operator/i.test(j.persona), description: 'Platform Engineering — reliability, performance, operations' },
  compliance: { label: 'Compliance', color: 'var(--cds-color-warning)', filter: j => /Compliance|Governance|Administrator|Audit/i.test(j.persona), description: 'Compliance & Governance — audit trails, policy enforcement' },
  data: { label: 'Data Team', color: 'var(--cds-color-success)', filter: j => /Data|Optimizer|Engineer|Onboard/i.test(j.persona), description: 'Data Engineering — pipeline optimization, onboarding, cost reduction' },
};
...
export default function SourceDetailPage() {
  ...
  const source = dataSources.flatMap(c => c.sources).find(s => s.id === sourceId);

  const routing = sourceId ? routingBlueprints[sourceId] ?? null : null;
  const secDetections = useMemo(() => sourceId ? secDetData[sourceId] ?? [] : [], [sourceId]);
  const obsDetections = useMemo(() => sourceId ? obsDetData[sourceId] ?? [] : [], [sourceId]);
  const enrichments = sourceId ? enrichmentData[sourceId] ?? null : null;
  const fields = useMemo(() => sourceId ? fieldMatrix[sourceId] ?? [] : [], [sourceId]);
  ...
  // Field analysis — above early return to satisfy rules-of-hooks
  const fieldAnalysis = useMemo(() => {
    const secRequired = new Set<string>();
    const obsRequired = new Set<string>();
    secDetections.filter(d => enabledSecDetections.has(d.id)).forEach(d => d.requiredFields.forEach(f => secRequired.add(f)));
    obsDetections.filter(d => enabledObsDetections.has(d.id)).forEach(d => d.requiredFields.forEach(f => obsRequired.add(f)));
    const allRequired = new Set([...secRequired, ...obsRequired]);
    const notNeeded = fields.map(f => f.field).filter(f => !allRequired.has(f));
    const guardProtected = fields.filter(f => f.guardAction && f.guardAction !== 'None').length;
  ...
  function toggleSecDetection(id: string) {
    setEnabledSecDetections(prev => { const n = new Set(prev); if (n.has(id)) { n.delete(id); } else { n.add(id); } return n; });
  }
  function toggleObsDetection(id: string) {
    setEnabledObsDetections(prev => { const n = new Set(prev); if (n.has(id)) { n.delete(id); } else { n.add(id); } return n; });
  }
  function selectAllSec() { setEnabledSecDetections(new Set(secDetections.map(d => d.id))); }
  function selectAllObs() { setEnabledObsDetections(new Set(obsDetections.map(d => d.id))); }
  ...
  function toggleStreamEnrichment(name: string) {
    setEnabledStreamEnrichments(prev => { const n = new Set(prev); if (n.has(name)) { n.delete(name); } else { n.add(name); } return n; });
  }
  function toggleSearchEnrichment(name: string) {
    setEnabledSearchEnrichments(prev => { const n = new Set(prev); if (n.has(name)) { n.delete(name); } else { n.add(name); } return n; });
  }
  function selectAllStreamEnrichments() { if (enrichments) setEnabledStreamEnrichments(new Set(enrichments.streamTime.map(e => e.name))); }
  function selectAllSearchEnrichments() { if (enrichments?.searchTime) setEnabledSearchEnrichments(new Set(enrichments.searchTime.map(e => e.name))); }
  ...
  async function exportSearchPack() {
    ...
    [...secDetections.filter(d => enabledSecDetections.has(d.id)),
     ...obsDetections.filter(d => enabledObsDetections.has(d.id))]
      .forEach(d => {
        if (d.criblSearchQueries) {
          d.criblSearchQueries.forEach((q, i) => {
            queries.push({
              id: `${d.id}_${i}`,
              name: `${d.name} - ${q.name}`,
    ...
    const blob = await buildSearchPack(
      { name: `${sourceId}-search-pack`, version: '1.0.0', displayName: `${source!.name} Search Pack`, description: `Search detections for ${source!.name}` },
      queries,
    );
    downloadBlobFile(blob, `${sourceId}-search-pack.crbl`);
    setExportStatus('Search Pack (.crbl) downloaded');
  }

  async function exportStreamPack() {
    const selectedEnrichments = enrichments?.streamTime?.filter(e => enabledStreamEnrichments.has(e.name)) ?? [];
    const blob = await buildStreamPack(
      { name: `${sourceId}-stream-pack`, version: '1.0.0', displayName: `${source!.name} Stream Pack`, description: `Stream pipeline for ${source!.name}` },
      {
        keepFields: [...fieldAnalysis.allRequired].sort(),
        removeFields: fieldAnalysis.notNeeded.sort(),
        enrichments: selectedEnrichments.map(e => ({ name: e.name, criblFunction: e.criblFunction, addedFields: e.addedFields })),
      },
    );
    downloadBlobFile(blob, `${sourceId}-stream-pack.crbl`);
    setExportStatus('Stream Pack (.crbl) downloaded');
  }

  async function deployToCriblSearch(dataset: string) {
    const apiUrl = (window as unknown as { CRIBL_API_URL?: string }).CRIBL_API_URL ?? '';
    ...
    for (const det of selectedDets) {
      ...
      for (const q of det.criblSearchQueries) {
        ...
        try {
          ...
          errorCount++;
          log(`! Error: ${cleanName} — ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    }
                ...
                <div style={{ ...card, marginBottom: 16 }}>
                  ...
                  <ul style={{ fontSize: 'var(--cds-font-size-sm)', color: 'var(--cds-color-fg-muted)', paddingLeft: 20, lineHeight: 2 }}>
                    ...
                    <li>Total queries: ~{
                      [...secDetections.filter(d => enabledSecDetections.has(d.id)),
                       ...obsDetections.filter(d => enabledObsDetections.has(d.id))]
                        .reduce((sum, d) => sum + (d.criblSearchQueries?.length || 0), 0)
                    } saved searches</li>
                  </ul>
          ...
          <div style={{ background: 'var(--cds-color-bg)', border: '1px solid var(--cds-color-border)', borderRadius: 'var(--cds-radius-xl)', padding: 32, maxWidth: 600, width: '90%', maxHeight: '80vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ ...card, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <button style={{ ...btnPrimary, whiteSpace: 'nowrap' }} onClick={exportSearchPack} disabled={totalEnabled === 0 && enabledSearchEnrichments.size === 0}>Download .crbl</button>
              </div>
              {/* Stream Pack */}
              <div style={{ ...card, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                ...
                <button style={{ ...btnPrimary, whiteSpace: 'nowrap' }} onClick={exportStreamPack}>Download .crbl</button>
              </div>
            </div>
                ...
                {source.jobsToBeDone.map(cat => (
                  <div key={cat.category} style={{ background: 'var(--cds-color-bg)', borderRadius: 'var(--cds-radius-md)', padding: 14, border: '1px solid var(--cds-color-border-subtle)' }}>
                    ...
                    <div style={{ fontSize: 'var(--cds-font-size-xs)', color: 'var(--cds-color-fg-muted)' }}>
                      {cat.jobs.length} outcome{cat.jobs.length > 1 ? 's' : ''} for {[...new Set(cat.jobs.map(j => j.persona))].join(', ')}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 12 }}>
                {source.jobsToBeDone
                  ...
                  .filter(cat => cat.filteredJobs.length > 0)
                  .map(cat => (
              ...
              {personaLens !== 'all' && source.jobsToBeDone.flatMap(c => c.jobs).filter(personaLenses[personaLens].filter).length === 0 && (
      ...
      {activeTab === 'detections' && hasDetections && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 16 }}>
            {(detectionSubTab === 'security' ? secDetections : obsDetections).map(d => {
      ...
      {activeTab === 'enrichments' && enrichments && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 16 }}>
            {(enrichmentSubTab === 'stream' ? enrichments.streamTime : enrichments.searchTime ?? []).map(e => {
        ...
        <div>
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {Array.isArray(routing.dataFlow) && routing.dataFlow.map(step => (
          ...
          {routing.packFunctions && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {routing.packFunctions.map((fn, i) => (
